...
class MetricsExporterAdapter(iMetricsExporter):
    ...
    def _initialize(self, application_name: str):
        try:
            ...
            self.exporter_endpoint = self.application_attributes.endpoints.get_metrics_endpoint()
            ...
            try:
                metric_exporter = OTLPMetricExporter(endpoint=self.exporter_endpoint, timeout=10)
                ...
                # Get meter instance
                self.meter = get_meter(f"meter-{self.application_attributes.application_name}", meter_provider=self.provider)
                ...
                self.meter = get_meter(f"fallback-{self.application_attributes.application_name}", meter_provider=self.provider)
        except Exception as e:
