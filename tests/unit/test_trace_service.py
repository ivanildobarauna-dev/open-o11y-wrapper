...
class TestTraceProcessorService:
    ...
    def test_new_span(self, mock_tracer):
        ...
        mock_tracer.start_span.assert_called_once_with(name="test-span", kind=SpanKind.INTERNAL, attributes={})
    ...
    def test_new_span_with_attributes(self, mock_tracer):
        ...
        mock_tracer.start_span.assert_called_once_with(name="test-span", kind=SpanKind.INTERNAL, attributes=attributes)
        assert span == mock_tracer.start_span.return_value
    ...
    def test_span_in_context(self, mock_tracer):
        ...
        with patch('otel_wrapper.domain.services.trace_service.set_span_in_context') as mock_set_context:
    ...
    def test_inject_context_into_headers(self):
        ...
        service._trace_context_propagator.inject.assert_called_once_with(carrier=headers, context=context)
        service._baggage_propagator.inject.assert_called_once_with(carrier=headers, context=context)
    ...
    def test_extract_context_from_headers(self):
        ...
        # Verify the propagators were called
        service._trace_context_propagator.extract.assert_called_once_with(carrier=headers)
        service._baggage_propagator.extract.assert_called_once_with(carrier=headers, context=extracted_context1)
