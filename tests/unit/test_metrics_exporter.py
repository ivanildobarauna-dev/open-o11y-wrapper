...
class TestMetricsExporterAdapter:
    ...
    @patch('otel_wrapper.infrastructure.adapters.metrics_exporter.ApplicationAttributes')
    @patch('otel_wrapper.infrastructure.adapters.metrics_exporter.Resource.create')
    @patch('otel_wrapper.infrastructure.adapters.metrics_exporter.PeriodicExportingMetricReader')
    ...
    def test_initialization(self, mock_get_meter, mock_meter_provider, mock_otlp_exporter, 
                           mock_metric_reader, mock_resource_create, mock_app_attrs):
        ...
        mock_app_attrs_instance.endpoints.get_metrics_endpoint.return_value = "http://test-endpoint:4317"
        ...
        with patch('otel_wrapper.infrastructure.adapters.metrics_exporter.MetricsExporterAdapter._instance', None):
            ...
            mock_otlp_exporter.assert_called_once_with(endpoint="http://test-endpoint:4317", timeout=10)
            mock_metric_reader.assert_called_once_with(exporter=mock_exporter)
            mock_meter_provider.assert_called_once_with(resource=mock_resource, metric_readers=[mock_reader])
    ...
    def test_metric_increment(self):
        ...
        # Setup mock objects
        with patch.object(MetricsExporterAdapter, '_initialize'), \
             patch('otel_wrapper.infrastructure.adapters.metrics_exporter.MetricsExporterAdapter._instance', None):
