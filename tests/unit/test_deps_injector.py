...
class TestDepsInjector:
    ...
    def test_wrapper_builder(self):
        ...
        with patch(
            "otel_wrapper.deps_injector.TraceExporterAdapter"
        ) as mock_trace_adapter, patch(
            "otel_wrapper.deps_injector.LogExporterAdapter"
        ) as mock_log_adapter, patch(
            "otel_wrapper.deps_injector.MetricsExporterAdapter"
        ) as mock_metrics_adapter, patch(
            "otel_wrapper.deps_injector.TraceProcessorService"
        ) as mock_trace_service, patch(
            "otel_wrapper.deps_injector.LogsProcessorService"
        ) as mock_log_service, patch(
            "otel_wrapper.deps_injector.MetricsProcessorService"
        ) as mock_metrics_service, patch(
            "otel_wrapper.deps_injector.Wrapper"
        ) as mock_wrapper:
            ...
            mock_trace_service.assert_called_once_with(
                trace_exporter=mock_trace_adapter_instance
            )
            mock_log_service.assert_called_once_with(
                log_exporter=mock_log_adapter_instance
            )
            mock_metrics_service.assert_called_once_with(
                metric_exporter=mock_metrics_adapter_instance
            )
...
@pytest.mark.skip(
    reason="This test creates actual singleton instances which may affect other tests"
)
